...
requirements-completed: [REQ-today-view, REQ-photo-resize, REQ-onboarding-empty-state, REQ-trajectory-line]
...
## Task 3: 창업자 iPhone 실기기 검증 (완료)

| # | 항목 | 결과 |
|---|------|------|
| 1 | 바텀시트 제스처 감각 (시트 CLOSED 상태에서 지도 팬 포함) | **PASS** |
| 2 | 저장된 핀 vs 진행 중 핀 시인성 | **PASS** (단, 별도 이견 — 아래 참고) |
| 3 | 고해상도 사진 첨부 체감 속도 | **PASS** |
| 4 | 체크인 진행 중 탭 전환 + 드래프트 유지 | **PASS** |

창업자가 "테스트 전부 확인했어"로 4개 항목 전체 통과를 명시적으로 확인함. 최초 설치 시 구 Dev Client(네이티브 모듈 미링크) 오인 설치로 `Cannot find native module 'ExpoImageManipulator'` 에러가 있었으나, Task 2가 만든 신규 EAS 빌드로 재설치 후 정상 동작 확인.

**항목 2 관련 이견 제기(테스트 체크리스트 항목이 아닌 별도 피드백):** 체크인 핀 색상이 지도에서 구분이 잘 안 될 만큼 흐리다는 의견. 색상 제한(빨간 계열 배제) 완화를 논의 중 — `docs/solutions` 또는 후속 gap-closure에서 다룰 사안.
