...
- **D-16:** 로그인 화면은 **Phase 10 백엔드 검증용으로만 존재** — 1단계(Phase 1~8) 앱
  ...
  결정.
- **D-16a (계획 단계 승인, 2026-09-02):** D-16의 개발자용 검증 화면은 **CLAUDE.md의
  DESIGN.md 적용 대상에서 명시적으로 제외**한다 — 계획 단계에서 UI 게이트가 트리거됐을
  때(UI-SPEC.md 없음) 창업자에게 직접 확인한 결과 `--skip-ui`로 진행하기로 명시적으로
  승인받음("1단계 제품 UI 미통합, DESIGN.md 적용 대상 아님" 근거). CLAUDE.md의
  "명시적 사용자 승인 없이 벗어나지 않는다" 요건은 이 승인으로 충족됨 — 단, 저장소
  전반의 토큰 사용 컨벤션(하드코딩 hex 색상 금지 등 `@/theme/tokens` 관례)은 그대로
  지킨다(별도 fence 테스트가 이미 강제하므로 무법 상태는 아님).
