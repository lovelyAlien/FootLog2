...
| 2 | 월 이동 2경로(헤더 화살표 + 스와이프) | ✅ 확인됨(버그 수정 후) | 최초 발견 시 헤더 화살표가 탭에 반응하지 않음(Bug 1, 아래) — 수정 후 화살표 탭·좌우 스와이프 둘 다로 월 이동 재확인(8월→7월→9월 왕복) |
...
| 5 | 스크러버 드래그(실시간 반영·관성 없음·경계 클램프) | ✅ 확인됨(버그 수정 후) | 최초 드래그 시 즉시 크래시(Bug 2, 아래) — 수정 후 재현: Sept 1→2로 실시간 이동, 오늘 경계에서 하드 클램프(더 넘어가지 않음), 손 뗀 자리에서 즉시 정지(관성 없음) 확인 |
...
**추가 수정 (2026-09-02, 창업자 승인 후 즉시 수정 — 커밋 8d1f9a7):**

- **Bug 1 수정 완료.** `CalendarGridScreen.tsx`에 `useSafeAreaInsets` 적용, `headerRow`에 `paddingTop: insets.top` 추가, 고정 `height: 44`를 `minHeight: 44`로 변경(내용 눌림 방지). 시뮬레이터에서 헤더 화살표 탭과 좌우 스와이프 둘 다 재확인.
- **Bug 2 수정 완료(2단계).** ① `scrubberRange.ts`의 `indexForTranslation`/`clampIndex`에 `'worklet'` 지시어 추가. ② 수정 후 재현 테스트 중 두 번째 크래시 발견 — 워클릿 함수의 기본 매개변수가 모듈 상수(`SCRUBBER_TICK_SPACING_PX`)를 참조하면 워클릿 클로저 캡처에서 빠짐(`Property 'SCRUBBER_TICK_SPACING_PX' doesn't exist`). `DateScrubber.tsx` 호출부에서 그 상수를 명시적으로 전달하도록 수정. 이후 크래시 없이 정상 동작(Sept 1→2 실시간 이동, 오늘 경계 하드 클램프) 확인.

**최초 발견 시 기록 (구현 변경 전, 참고용 — 둘 다 위에서 수정 완료):**
