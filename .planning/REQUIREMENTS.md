...
- [x] **REQ-reflection-autosave** (M24, DE-T2): 회고 답변은 5초 디바운스, `AppState` 백그라운드 flush, 모달 닫기(✕/스와이프) flush — 모두 동일한 저장 함수를 통해 — 자동저장된다. (07-02 autosaveController + 07-04 useReflectionDraft + 07-05 ReflectionModal 배선 조합으로 완전 충족 — orchestrator가 wave 5 병합 후 코드 직접 확인: `useReflectionDraft.ts`의 AppState 리스너/날짜전환·언마운트 flush, `ReflectionModal.tsx`의 닫기 시 flush→router.back())
- [x] **REQ-reflection-save-failure-ui** (M25, DE-T1): 회고 저장 실패는 체크인 저장 실패와 동일한 재시도 패턴을 사용한다. (07-02 reflectionRepo.runWithSingleRetry + 07-04 ReflectionPrompts 에러 UI + 07-05 ReflectionModal의 `onRetry={draft.onRetry}` 배선으로 완전 충족 — orchestrator가 wave 5 병합 후 코드 직접 확인)
