...
- [ ] **REQ-reflection-notification** (M27, DE-T4; M4에 의존): 매일 고정 시각의 회고 알림이 기본으로 켜져 있으며, 설정에서 끌 수 있는 토글이 있다. 설정 화면에서 회고 알림 시각을 직접 선택할 수 있다(후보: 19/20/21/22/23시, 기본 21시) — (원본 문서 `day-end-reflection-map.md` Premise #4는 "시각 변경 UI는 스코프 밖"이었으나, 2026-09-02 창업자 논의(D-05)에서 명시적으로 뒤집혀 이번 phase 스코프로 편입됨).
