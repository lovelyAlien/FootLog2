...
### 정각 반복 알림 실제 수신 + 자가진단 재생성 로그 실기기 확인 (P1)
**What:** 02-08 실기기 검증에서 2~5단계(priming 화면, 권한 거부 배너, 설정 딥링크 왕복)는 창업자가 확인했지만, 6단계(정각에 `체크인할 시간이에요` 알림이 실제로 반복 수신되는지, 최대 1시간 대기 필요)와 7단계(설정에서 알림을 껐다 켠 뒤 콘솔에 자가진단 재생성 로그가 남는지)는 시간 제약으로 검증하지 못했다. 1~2주 트라이얼 기간 중 직접 관찰해 확인 필요.
**Why:** REQUIREMENTS.md의 REQ-notification-scheduling과 REQ-notification-denied-flow는 이 두 단계가 실기기에서 확인돼야 Complete로 전환할 수 있다 — 현재 Pending 유지 중.
**Pros:** 1~2주 트라이얼의 "알림을 안 탭했다" 관찰을, 무시해서인지 애초에 안 울려서인지 구분할 기준선이 된다(day-end-reflection-map.md §측정 교란 요인).
**Cons:** 없음 — 시간이 걸릴 뿐인 관찰 작업.
**Context:** 02-08-SUMMARY.md Issues Encountered 참고. 7단계는 코드 분석 결과 단순히 iOS 설정에서 알림을 껐다 켜는 것만으로는 재생성 로그가 안 뜰 수 있음이 확인됨(`registry.ts`의 `runForegroundNotificationCheck`는 권한이 없으면 자가진단을 건너뛰고, 권한이 다시 켜져도 실제로 빠진 트리거가 있을 때만 로그를 남김 — iOS가 권한을 끈다고 이미 예약된 로컬 알림을 자동 취소하진 않으므로 "빠진 게" 애초에 없을 수 있다). 확실히 재현하려면 앱 재설치가 더 나은 방법일 수 있다.
**Depends on/blocked by:** 없음 — 다음 정각/트라이얼 기간에 언제든 확인 가능.
